...
<style>
  ...
  h2 + dl { /* only the `dl`s in each chapter */
    margin: 0 -1rem 10em 0;
  }
  @media (min-width: 50em) { /* em not rem */
    h2 + dl { /* the `dl`s in each chapter... */
      font-size: 2em; /* em not rem */
    }
  }
  h2 + dl dt { /* "left side": trigram and reading */
    float: left;
    width: 50%;
    margin-top: 0;
  }
  h2 + dl dt:first-child { /* trigram */
    padding: 2rem 0.5rem;
    font-size: 5rem; /* rem not em...? */
    line-height: 0;
    text-align: center;
  }
  h2 + dl dt:not(:first-child) { /* reading */
    display: flex;
  ...
  h2 + dl dd { /* "right side": TP interpretations */
    width: 49%;
    margin-left: 50%;
  ...
  h2 + dl dd strong, /* any one-off english... */
  h2 + dl [lang=en] {
    font: 0.9em / 1.5em sans-serif; /* em not rem */
  }
  h2 + dl dd:first-of-type { /* sitelen-sitelen pronunciation */
    padding-left: 7%;
  }
  h2 + dl dd:nth-of-type(2),  /* kipisi lawa pi linja mute */
  h2 + dl dd:nth-of-type(3) { /* kipisi noka pi linja mute */
  ...
  h2 + dl dt:not(:first-of-type) ~ dd { /* TP interpretation all lines */
    padding-left: 1em; text-indent: -1em; /* indents the wrapped lines... */
  }
  h2 + dl dt:not(:first-of-type) + dd { /* first line of interpretation */
