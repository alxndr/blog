...
slug: 2023/05/11/recursive-techdebt-upgrade-ticket
tags:
  - agile
  - techdebt
  - dependencies
...
The ticket is titled "Upgrade or refactor some dependencies", or something <em style="text-shadow:1px 1px 1px red">intentionally slightly vague</em> — This is to allow the developer some leeway in deciding what sorts of dependency work they'll tackle, each time the ticket rolls around.

The ticket is scheduled for a regularly-occurring <em style="text-shadow:1px 1px 1px orange">specific time period</em> —  sprint, month, whatever the team is already using — This time period should be regular enough that the ticket doesn't feel unusual, but not so frequent that it gets in the way of other work.

This ticket is actually <em style="text-shadow:1px 1px 1px magenta">many tickets, which are created recursively</em> — part of the Acceptance Criteria is to create a new ticket, ready to be scheduled/refined — This ensures that the pattern continues!
